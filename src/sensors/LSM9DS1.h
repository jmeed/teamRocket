...
public:
	...
	float read_mag_gs(uint8_t dimension);
	...
	void set_gyro_scale(gyro_scale g_sc);
	...
	void configure_gyro_int(uint8_t int1_cfg, uint16_t int1_ths_x,
	...
	// int1_ths_z = 16 bit interrupt threshold value for z-axis
	// duration = duration an interrupt holds after triggering
	void configure_accel_int(uint8_t int1_cfg, uint8_t int1_ths_x,
	...
	// duration = duration an interrupt holds after triggering
	void configure_mag_int(uint8_t int1_cfg, uint16_t int1_ths);
...
private:
	...
	int8_t read_reg_xlg(uint8_t reg_addr);

	// Write a register on the accel/gyro device
	void write_reg_xlg(uint8_t reg_addr, uint8_t data);

	// Read a register on the magnetometer device
	int8_t read_reg_mag(uint8_t reg_addr);
