...
static void i2c_write_test(I2C_XFER_T *xfer)
{
	int tmp, i;
...
int main(void)
{
	...
	uint8_t I2CREAD = 0xd7;
	uint8_t I2CWRITE = 0xd6;
...
//	tx_buff[0] = 0x26;
//	while (1) {
//		Chip_I2C_MasterSend(I2C0, (0xD4>>1), tx_buff, tx_size);
//		Chip_I2C_MasterRead(I2C0, (0xD6>>1), rx_buff, rx_size);
//		//temp = gyro.read_data(L3G_TEMPERATURE);
//		//DEBUGOUT("%.2f", temp);
//		for (i = 0; i < 500000; ++i);
//	}
//
...
// to read temperature, should not be needed
//void setup_xfer(I2C_XFER_T *xfer) {
//	//uint8_t tx_buf[tx_sz];
//	//uint8_t rx_buf[rx_sz];
//
//	//tx_buf[0] = L3G_OUT_TEMP;
//
//	xfer->slaveAddr = (0xD4 >> 1);
//	xfer->txBuff = 0;
//	xfer->rxBuff = 0;
//	xfer->txSz = 1;
//	xfer->rxSz = 1;
//	buffer[0][0] = L3G_OUT_TEMP;
//	xfer->txBuff = buffer[0];
//	xfer->rxBuff = buffer[1];
//}
