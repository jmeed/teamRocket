...
void LSM303::enable() {
	...
	int n = write_reg(CTRL_REG2, 0x20);
	
	// 0x57 = 0b01010111
	// ADDR = 0101 (50 Hz ODR), AZEN = AYEN = AXEN = 1 (all axes enabled)
	n = write_reg(CTRL_REG1, 0x57);
	...
	n = write_reg(CTRL_REG6, 0x20);

	// 0x00 = 00000000
	// MLP = 0 (low power mode off), MD = 00 (continuous-conversion mode)
	n = write_reg(CTRL_REG7, 0x00);
}
...
int16_t LSM303::read_accel_raw(uint8_t dimension) {
	...
	switch (dimension) {
		case ACCEL_X:
			reg_addr_l = OUT_X_L_A;
			reg_addr_h = OUT_X_H_A;
			break;
		case ACCEL_Y:
			reg_addr_l = OUT_Y_L_A;
			reg_addr_h = OUT_Y_H_A;
			break;
		case ACCEL_Z:
			reg_addr_l = OUT_Z_L_A;
			reg_addr_h = OUT_Z_H_A;
			break;
		default:
			reg_addr_l = OUT_X_L_A;
			reg_addr_h = OUT_X_H_A;
