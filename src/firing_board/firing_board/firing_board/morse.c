...
#define LED_OFF() { memset(channel_colors, 0, sizeof(channel_colors)); neopixel_update(); }
#define LED_ON() { uint8_t i; LED_OFF(); for(i = 0; i < CHANNELS; i++) channel_colors[i].red = 0x1f; neopixel_update(); }
...
static const BYTE morse_data[26] PROGMEM =
...
static const BYTE morse_num_data[10] PROGMEM =
{
...
static void morse_char_play( char c ) {
	...
	if (c >= 'a' && c <= 'z') c += 'A' - 'a';
	if (c >= '0' && c <= '9') {
		byProgram = pgm_read_byte(&morse_num_data[c - '0']);
	} else if (c >= 'A' && c <= 'Z') {
		byProgram = pgm_read_byte(&morse_data[c - 'A']);
	} else {
        DELAY_MS( T_SPACE );
        return;
	}
