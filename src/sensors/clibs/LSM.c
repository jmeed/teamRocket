...
bool LSM_init(I2C_ID_T id_in,
	...
	LSM_set_gyro_scale(g_scale);	// Set range
	
	// Accel initialization
	LSM_init_accel();
	LSM_set_accel_odr(a_odr);
	LSM_set_accel_scale(a_scale);

	// Mag initialization
	LSM_init_mag();
	LSM_set_mag_odr(m_odr);
	LSM_set_mag_scale(m_scale);
...
void LSM_configure_gyro_int(uint8_t int1_cfg, uint16_t int1_ths_x,
	...
	if (duration)
		LSM_write_reg_xlg(LSM_INT1_GEN_DUR_G, 0x80 | duration);
	else
		LSM_write_reg_xlg(LSM_INT1_GEN_DUR_G, 0x00);
}
...
void LSM_configure_accel_int(uint8_t int1_cfg, uint8_t int1_ths_x,
	...
	if (duration)
		LSM_write_reg_xlg(LSM_INT1_GEN_DUR_XL, 0x80 | duration);
	else
		LSM_write_reg_xlg(LSM_INT1_GEN_DUR_XL, 0x00);
}
...
void LSM_init_gyro() {
	...
	LSM_write_reg_xlg(LSM_CTRL_REG4_G, 0x38);
}
