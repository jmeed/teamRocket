...
float L3G_read_data(uint8_t dimension) {
	switch (dimension) {
		case L3G_TEMPERATURE:
			return L3G_read_temperature_C();
		default:
			return L3G_read_spin_rate_dps(dimension);
	}
}
...
void L3G_enable() {
	...
	L3G_write_reg(L3G_CTRL_REG1, 0x0F);

	// 0x20 = 0b00100000
	// 2000 dps mode
	L3G_write_reg(L3G_CTRL_REG4, 0x20);
}
...
int16_t L3G_read_spin_rate_raw(uint8_t dimension) {
	...
	r_l = L3G_read_reg(reg_addr_l);
	r_h = L3G_read_reg(reg_addr_h);
	return (int16_t)(r_h << 8 | r_l);
}

float L3G_read_spin_rate_dps(uint8_t dimension) {
	return (float)L3G_read_spin_rate_raw(dimension) * (2000.0f / 32768.0f);
}

uint8_t L3G_read_temperature_raw() {
	return L3G_read_reg(L3G_OUT_TEMP);
}

float L3G_read_temperature_C() {
	return 42.5f + (float)L3G_read_temperature_raw() / 480.0f;
}
